...
_ARTICLE: ArticleRecord = {
    "doi": "10.1017/test001",
    "journal": "APSR",
    "title": "Colonial Institutions and Development",
    "year": 2020,
    "month": None,
    "volume": None,
    "issue": None,
...
def test_correction_updates_regions(conn: sqlite3.Connection, tmp_path: Path) -> None:
    ...
    path = _write_overrides(
        tmp_path,
        {
            "corrections": [{"doi": _ARTICLE["doi"], "regions": ["Eastern Europe"]}],
            "additions": [],
        },
    )
...
def test_correction_skips_unknown_doi(conn: sqlite3.Connection, tmp_path: Path) -> None:
    path = _write_overrides(
        tmp_path,
        {
            "corrections": [{"doi": "10.9999/unknown", "regions": ["Eastern Europe"]}],
            "additions": [],
        },
    )
    corr, adds = apply_overrides(conn, path)
...
def test_correction_only_writes_present_fields(
...
def test_correction_is_hpe_false(conn: sqlite3.Connection, tmp_path: Path) -> None:
    ...
    path = _write_overrides(
        tmp_path,
        {"corrections": [{"doi": _ARTICLE["doi"], "is_hpe": False}], "additions": []},
    )
    apply_overrides(conn, path)
    row = conn.execute(
        "SELECT is_hpe FROM classifications WHERE doi = ?", (_ARTICLE["doi"],)
    ).fetchone()
    assert row[0] == 0
...
def test_correction_idempotent(conn: sqlite3.Connection, tmp_path: Path) -> None:
    ...
    path = _write_overrides(
        tmp_path,
        {
            "corrections": [{"doi": _ARTICLE["doi"], "regions": ["Northern Europe"]}],
            "additions": [],
        },
    )
    apply_overrides(conn, path)
...
def test_addition_inserts_new_article(conn: sqlite3.Connection, tmp_path: Path) -> None:
    ...
    assert adds == 1
    row = conn.execute(
        "SELECT title FROM articles WHERE doi = ?", ("10.1017/new001",)
    ).fetchone()
    assert row[0] == "New Paper"
    url_row = conn.execute(
        "SELECT replication_url FROM classifications WHERE doi = ?", ("10.1017/new001",)
    ).fetchone()
    assert url_row[0] == _URL


def test_addition_existing_doi_overwrites(
    conn: sqlite3.Connection, tmp_path: Path
) -> None:
    upsert_article(conn, _ARTICLE)
    upsert_classification(conn, _cls(_ARTICLE["doi"]))
...
def test_addition_minimal_fields_succeeds(
    conn: sqlite3.Connection, tmp_path: Path
) -> None:
    """A dataset need not be tied to a journal/paper: an addition with only
    doi/title/replication_url is applied, with safe defaults filled in."""
    path = _write_overrides(
        tmp_path,
        {
            "corrections": [],
            "additions": [
                {
                    "doi": "10.1017/minimal",
                    "title": "Standalone Dataset",
                    "replication_url": _URL,
                    # journal, year, regions, countries, period_*, is_hpe all omitted
                }
            ],
        },
    )
    ...
    assert adds == 1
    art = conn.execute(
        "SELECT journal, year FROM articles WHERE doi = ?", ("10.1017/minimal",)
    ).fetchone()
    assert art[0] == "Unpublished"  # placeholder for the NOT NULL journal column
    assert art[1] is None
    cls = conn.execute(
        "SELECT is_hpe, regions, countries, replication_url FROM classifications WHERE doi = ?",
        ("10.1017/minimal",),
    ).fetchone()
    assert cls[0] == 1  # additions default to HPE
    assert json.loads(cls[1]) == []
    assert json.loads(cls[2]) == []
    assert cls[3] == _URL


def test_addition_missing_doi_or_url_skipped(
    conn: sqlite3.Connection, tmp_path: Path
) -> None:
    """Genuinely required fields (title + replication_url) still gate additions."""
    path = _write_overrides(
        tmp_path,
        {
            "corrections": [],
            "additions": [
                {"title": "No URL", "replication_url": None},  # no doi
                {"doi": "10.1017/nourl", "title": "No URL"},  # no replication_url
                {"doi": "10.1017/notitle", "replication_url": _URL},  # no title
            ],
        },
    )
    corr, adds = apply_overrides(conn, path)
    ...
    assert (
        conn.execute(
            "SELECT 1 FROM articles WHERE doi = ?", ("10.1017/nourl",)
        ).fetchone()
        is None
    )
    assert (
        conn.execute(
            "SELECT 1 FROM articles WHERE doi = ?", ("10.1017/notitle",)
        ).fetchone()
        is None
    )


def test_addition_abstract_stored(conn: sqlite3.Connection, tmp_path: Path) -> None:
    path = _write_overrides(
        tmp_path,
        {
            "corrections": [],
            "additions": [
                {
                    ...
                    "replication_url": _URL,
                    "abstract": "Panel of grain prices and harvest failures, 1500-1800.",
                }
            ],
        },
    )
    ...
    assert adds == 1
    row = conn.execute(
        "SELECT abstract FROM articles WHERE doi = ?", ("10.1017/withabs",)
    ).fetchone()
    assert row[0] == "Panel of grain prices and harvest failures, 1500-1800."


def test_addition_unknown_journal_succeeds(
    conn: sqlite3.Connection, tmp_path: Path
) -> None:
    ...
    assert adds == 1
    row = conn.execute(
        "SELECT journal FROM articles WHERE doi = ?", ("10.1017/new999",)
    ).fetchone()
...
def test_correction_missing_doi_is_skipped(
    conn: sqlite3.Connection, tmp_path: Path
) -> None:
    path = _write_overrides(
        tmp_path, {"corrections": [{"regions": ["Eastern Europe"]}], "additions": []}
    )
    corr, adds = apply_overrides(conn, path)
    assert corr == 0


def test_correction_no_correctable_fields_is_noop(
    conn: sqlite3.Connection, tmp_path: Path
) -> None:
    upsert_article(conn, _ARTICLE)
    upsert_classification(conn, _cls(_ARTICLE["doi"]))
...
def test_verbose_output_does_not_crash(
    conn: sqlite3.Connection, tmp_path: Path, capsys: pytest.CaptureFixture
) -> None:
    upsert_article(conn, _ARTICLE)
    upsert_classification(conn, _cls(_ARTICLE["doi"]))
    path = _write_overrides(
...
def test_addition_missing_doi_is_skipped(
    conn: sqlite3.Connection, tmp_path: Path
) -> None:
    path = _write_overrides(
        tmp_path,
...
def test_addition_authors_not_list_does_not_crash(
    conn: sqlite3.Connection, tmp_path: Path
) -> None:
    path = _write_overrides(
        tmp_path,
    ...
    assert (
        conn.execute(
            "SELECT 1 FROM articles WHERE doi = ?", ("10.1017/badauthors",)
        ).fetchone()
        is not None
    )
